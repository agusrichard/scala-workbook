...
def post(id):
	...
	page = request.args.get('page', 1, type=int)
	if page == -1:
		page = (post.comments.count() - 1) // current_app.config['TALOS_COMMENTS_PER_PAGE'] + 1
	pagination = post.comments.order_by(Comment.timestamp.asc()).paginate(
		page, per_page=current_app.config['TALOS_COMMENTS_PER_PAGE'],
		error_out=False)
...
def moderate():
	...
	return render_template('main/moderate.html', comments=comments,
		pagination=pagination, page=page, title='Moderate')
